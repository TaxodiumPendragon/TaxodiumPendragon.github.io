---
title: 期中复习：2024—2025 考后分析
description: 2024 与 2025 年期中试卷分析、算法设计题和复习提纲。
course: data-structures-and-algorithms
category: review
order: 3
source_file: Mid/Mid.md
tags:
  - midterm
  - exam
sidebar:
...
> 25期中：扣分部分是填空题完全二叉树与循环队列、算法填空里某个指向head的指针、算法设计里快慢指针写错了，树边界条件没考虑

- 填空题是完全二叉树子树忘了+2，没有仔细观察检索标志是什么以及代入验证
- 另一个循环队列，忘了应该从front指向的位置来写出答案。顺序的存在是变动的，应该考虑如果写一个输出的程序会输出什么
- 代码填空那个head指针也是忘了，没有仔细考虑会实现出的数据结构吧
- 算法设计
- - 快慢指针的程序完全写错了
- - 伪代码里输出和return是两个意思，递归里不该直接写输出
- - 二叉树那里没考虑空树的边界条件，既然使用指针就应该
...
void postOrderTraversal(Node**root)//后根遍历
{
  ...
  st.push(stackElement(root,0));
  while(!st.empty()){
    stackElement topElem=st.top();
    st.pop();
    if(topElem.node->getChildrenNum()==0||topElement.rd==1)//rd是回访标记,可以在前文的数据结构里看到，1代表孩子已处理
      Visit(topElem.node);
    else{
      st.push(stackElement(topElem.node,1));
      for(int i=0;i<topElem.node->getChildrenNum();++i){
        child=topElem.node->getKthChild(topElem.node->getChildrenNum()-1-i);
        st.push(stackElement(child,0);)
      }
    }
}
}
